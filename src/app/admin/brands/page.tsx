...
export default function AdminBrands() {
  const [brands, setBrands] = useState<(Brand & { id: string | number })[]>([])
  ...
  const fetchBrands = async () => {
    ...
    try {
      ...
      const items: (Brand & { id: string | number })[] = []
      querySnapshot.forEach((docSnap) => {
        items.push({ id: docSnap.id, ...docSnap.data() } as (Brand & { id: string }))
      })
      setBrands(items)
    } catch (err) {
  ...
  const handleDelete = async (id: string | number) => {
    ...
    try {
      await deleteDoc(doc(db, 'brands', id.toString()))
  ...
  const handleEdit = (brand: Brand & { id: string | number }) => {
    setFormData(brand)
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await setDoc(doc(db, 'brands', editingId.toString()), formData)
        setBrands(brands.map(b => b.id === editingId ? { id: editingId, ...formData } as (Brand & { id: string }) : b))
      } else {
        const docRef = await addDoc(collection(db, 'brands'), formData)
        setBrands([...brands, { id: docRef.id, ...formData } as (Brand & { id: string })])
      }
      handleCancel()
    } catch (err) {
