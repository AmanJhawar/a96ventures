...
export default function AdminInventory() {
  ...
  const handleEdit = (item: InventoryItem) => {
    const hasVariants = (item.standardSizes?.length > 0 || item.customSizes?.length > 0) && (item.standardPurities?.length > 0 || item.customPurities?.length > 0)
                ...
                {(() => {
                  ...
                  if (combos.length === 0) return (
                    <div className="mt-8 pt-6 border-t border-gray-200">
                      <p className="text-sm text-gray-500 italic">Select at least one Size and one Purity to configure variant SKUs and weights.</p>
                    </div>
                  )
