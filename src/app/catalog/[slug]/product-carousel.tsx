...
export function ProductCarousel({ images, productName }: ProductCarouselProps) {
  ...
  return (
    <div className="flex flex-col gap-4 w-full">
      {/* Main Image */}
      <div className="aspect-square bg-white rounded-2xl border border-gray-200 overflow-hidden relative shadow-sm transition-all duration-300">
        <ProtectedImage
          ...
          {safeImages.map((img, idx) => {
            ...
            return (
              <button
                ...
                className={`relative flex-shrink-0 w-20 h-20 md:w-24 md:h-24 rounded-xl overflow-hidden border-2 transition-all duration-200 ease-[var(--ease-out)] snap-start outline-none focus-visible:ring-2 focus-visible:ring-black focus-visible:ring-offset-2 ${
                  isActive 
